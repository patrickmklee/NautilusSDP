...
void dive(int percent) { //Use negative percent to rise
	printf("trying to dive\n");
/*	int pulse;
	pulse = NEUTRAL_THROTTLE+(500*percent)/100;
	int pulseRev = NEUTRAL_THROTTLE-(500*percent)/100;
	gpioServo(PIN_LTOP, pulseRev);
...
void *receiveCmds(void * mArgs) {
   ...
   while (1) {
      /*if (x_correction != mCtrlArgs->roll_correction){
	      x_correction = mCtrlArgs->roll_correction;
	      //printf("Motor Control x_correction=%0.2f\n",x_correction);
      }*/
...
void *receiveCmds(void * mArgs) {
   struct TopFeedbackStateStruct *TopSetpoint = (struct TopFeedbackStateStruct *)mArgs;
   struct AxisFeedbackState *DepthSetpoint = TopSetpoint->zState;   
   char wr_cmd[2];
   strcpy(wr_cmd, "FD");
   char keyup_cmd[2];
   char keydown_cmd[2];
